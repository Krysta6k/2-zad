  ...
  <div class="container h-100">
    <div class="auth__column form-column d-flex justify-content-center flex-columns h-100 align-items-center">
      <div class="auth-form w-100" style="max-width:520px;text-align:center">
        <h3 class="auth-form__title mb-4">Создать аккаунт</h3>

        <form class="form" action="<?= $results['form']['action'] ?>" method="post">
          <input type="hidden" name="redirect" value="<?= $_GET['redirect'] ?>">

          <div class="auth-form">
            <input class="form__field form-control mb-3" name="user[name]" type="text" required id="name" placeholder="Введите свое имя">
            <input class="form__field form-control mb-3" name="user[phone]" type="phone" required id="phone" placeholder="Ваш номер телефона">

            <button class="btn btn-primary mb-3 auth-form__button login-btn" type="submit" name="<?= $results['form']['btn'] ?>">Зарегистрироваться</button>
            ...
            <div class="auth-btn-wrapper">
              <a href="/user/login" class="create-account-btn text-muted d-block">Уже есть аккаунт</a>
