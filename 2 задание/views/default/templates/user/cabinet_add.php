<form action="/user/add" method="post" enctype="multipart/form-data">
  ...
  <div class="form-group mb-3">
    <div class="img preview embed-responsive mb-3">
      <label for="upload" rel="tooltip" title="Выбрать изображение" class="holder bg-light border-0 rounded text-center d-flex h-100 w-100 justify-content-center align-items-center" style="cursor: pointer; aspect-ratio: 1/1; background-repeat: no-repeat; background-size: cover; background-image: url(<?= json_decode($results['user']->img)->fullsize ?>)">
    ...
    <input type="hidden" name="user[img]" class="form-control mb-3" autocomplete="off" value="<?= $results['user']->img ?>">
  </div>
  ...
  <div class="form-group mb-3">
	...
	<div class="form-group">
		...
		<input type="datetime-local" name="user[registration_date]" class="form-control mb-3" min="<?= date('Y-m-d H:i:s') ?>" value="<?= date('Y-m-d H:i:s') ?>" autocomplete="off" required readonly>
	</div>

	<div class="form-group">
		<label class="text-muted mb-2"><?= Lang::get('titles', 'Token') ?></label>
		<input type="text" name="user[code]" class="form-control mb-3" autocomplete="off" value="<?= md5(random_bytes(64)) ?>" required readonly>
	</div>
  ...
  <div class="form-group mb-3">
    ...
    <div class="input-group mb-2">
      <input type="password" name="user[password]" class="password form-control" required>
  ...
  <button type="submit" name="add" class="btn btn-danger lh-sm py-2 px-5 rounded-5 my-4 w-100"><?= Lang::get('btns', 'Add') ?></button>
</form>
